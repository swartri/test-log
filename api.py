...
st.set_page_config(page_title="Fe Log Analysis Tool", layout="wide")
...
# Pre-compile Regex outside the function
time_taken_regex = re.compile(r'Time Taken in ms= (\d+)')

def extract_details_with_timestamp(data, url_filter, context_depth):
    app_context_regex_template = '^/'
    if context_depth > 0:
        app_context_regex_template += '([^/, ]+/){1,' + str(context_depth) + '}'
    app_context_regex = re.compile(app_context_regex_template.rstrip('/'))
    ...
    for index, row in data.iterrows():
        ...
        app_context_match = re.match(app_context_regex_template, url)
